...
\paragraph{Preliminary evaluation windows.} This is an exploratory scan with a small number of trajectories per window ($n_{\text{traj}}{=}10$ early, $n_{\text{traj}}{=}5$ paper), not a saturated benchmark; we treat the resulting numbers as preliminary characterization. We tested both:
\begin{itemize}
  \setlength\itemsep{0.2em}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/fig_per_traj.pdf}
  \caption{Per-trajectory rollout for the four models, overlaid on a single axes (10 trajectories per model, $\log$-$y$). Walrus's divergence (red) is structural across all 10 trajectories rather than outlier-driven, while the three FNO baselines remain bounded. The legend identifies configurations.}
  \label{fig:per-traj}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Perpendicular-cascade evolution for $B_y$ at rollout steps 1, 5, 10, 25, 50. Walrus zero-shot (red, solid) departs from truth (black) progressively, with energy preferentially piling up at high $k_\perp$ by step 50. The high/low-$k$ ratio collapses from $\sim$150 (truth-like) at step~1 to $\sim$3 at step~50.}
...
\begin{figure}[t]
  ...
  \caption{Perpendicular-cascade evolution for $B_z$ at the same rollout steps. The same high-$k$ pile-up is visible on trajectories whose basin selects $B_z$; trajectories that instead select $B_y$ leave $B_z$ near truth.}
