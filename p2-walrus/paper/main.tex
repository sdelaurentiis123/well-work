...
\begin{itemize}
  \setlength\itemsep{0.2em}
  \item \textbf{Early window (preliminary):} input frames $[0,1,2]$ $\to$ predict frames $[3,\ldots,52]$, $K{=}50$, $n_{\text{traj}}{=}10$. We report this as a preliminary characterization (it does not match the paper's $T{\geq}17$ rollout convention); the paper window below is the primary evaluation.
...
\begin{figure}[t]
  ...
  \caption{Per-trajectory results. All four configurations are overlaid on a single log-$y$ axes. Thick lines mark per-configuration means; thin lines are individual trajectories, colored by configuration as indicated in the legend. Walrus's divergence is structural across all 10 trajectories rather than outlier-driven, while every FNO baseline stays bounded.}
  \label{fig:per-traj}
\end{figure}
