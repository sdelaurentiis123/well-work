...
\paragraph{Critical scope correction.} Walrus's published MHD T21--60 = 1.2256 in Table~1 of~\cite{mccabe2025walrus} refers to a model \emph{fine-tuned on MHD} per the cross-domain analysis protocol of Section~5.2 of that paper (an additional 500K-sample fine-tune on top of the base pretrained checkpoint). 

The paper \emph{does} plot the corresponding zero-shot rollout curve in Figure~15 (Appendix E.4.1), labeled ``Walrus-PT'', and the MHD~(3D) panel of that figure shows the zero-shot VRMSE climbing from $\sim 1$ at step~1 to roughly $10^6$ by step 100 --- consistent with our findings here. The paper's own Table~7 lists MHD~(3D) as one of only three of nineteen pretraining datasets with median trajectory-averaged VRMSE $\geq 10$ \emph{with} the patch-jitter stability mechanism enabled. The catastrophic zero-shot divergence on MHD is therefore not in dispute; the paper's own data establishes it. \emph{What this work adds is the physics-feature decomposition of that failure mode, not the observation that it exists.} The natural follow-up --- running the Sec.~5.2 fine-tune protocol on MHD and applying this diagnostic suite to the result --- is future work.
...
\paragraph{Preliminary: two evaluation windows.} As a preliminary characterization, we evaluated on two small trajectory samples:
\begin{itemize}
  \setlength\itemsep{0.2em}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\linewidth]{figures/fig_per_traj.pdf}
  \caption{Per-trajectory rollout VRMSE: all 10 trajectories for each of the four configurations overlaid on a single panel. Method is encoded by color/line style (legend); thin traces are individual trajectories and the thicker line of each color is the across-trajectory median. Walrus's divergence is structural across all 10 trajectories rather than outlier-driven.}
  \label{fig:per-traj}
\end{figure}
...
Figures~\ref{fig:cascade-evolution-by} and~\ref{fig:cascade-evolution-bz} show the perpendicular cascade $E(k_\perp | k_\parallel \to 0)$ for $B_y$ and $B_z$ at multiple rollout steps. Walrus's predicted spectrum at step~1 is truth-like (low-$k$ dominated, $\sim$150$\times$ above high-$k$). As rollout progresses, the spectrum flattens: by step 50, low/mid/high-$k$ energies are within $\sim$1 order of magnitude.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/fig_cascade_evolution_By.pdf}
  \caption{Perpendicular-cascade evolution for $B_y$ at rollout steps 1, 5, 10, 25, 50. Walrus zero-shot (red, solid) departs from truth (black) progressively, with energy preferentially piling up at high $k_\perp$ by step 50.}
  \label{fig:cascade-evolution-by}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/fig_cascade_evolution_Bz.pdf}
  \caption{Perpendicular-cascade evolution for $B_z$ at rollout steps 1, 5, 10, 25, 50. Same configurations and color coding as Fig.~\ref{fig:cascade-evolution-by}; the high/low-$k$ ratio collapses from $\sim$150 (truth-like) at step~1 to $\sim$3 at step~50.}
