...
\paragraph{Critical scope correction.} Walrus's published MHD T21--60 = 1.2256 in Table~1 of~\cite{mccabe2025walrus} refers to a model \emph{fine-tuned on MHD} per the cross-domain analysis protocol of Section~5.2 of that paper (an additional 500K-sample fine-tune on top of the base pretrained checkpoint). Polymathic has released only the base pretrained checkpoint (\texttt{polymathic-ai/walrus} on HuggingFace), not the per-task fine-tuned variants.
