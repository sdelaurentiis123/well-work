...
\paragraph{Reframed comparison to the Walrus paper.} Walrus's published Table~1 MHD T21--60 = 1.2256 is for a fine-tuned model. Our zero-shot T21--60 median = 18.25 (paper window, $n_{\text{traj}}{=}5$). The 14$\times$ ratio is an apples-to-oranges comparison; we report it only to note that the paper's own Section~5.2 protocol (additional 500K-sample fine-tune) is the published path to MHD-deployable Walrus. Whether that fine-tuning protocol resolves the basin instability \emph{or} the instability survives is the most informative single follow-up experiment available.
