...
\paragraph{Preliminary evaluation windows.} The windowing choices below are exploratory and subject to revision. We tested both:
\begin{itemize}
  \setlength\itemsep{0.2em}
...
\begin{figure}[t]
  ...
  \caption{Per-trajectory rollout. Walrus's divergence (bottom-right panel) is structural across all 10 trajectories rather than outlier-driven. Step-1 statistics in subtitles confirm Walrus has the best one-step prediction.}
  \label{fig:per-traj}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/fig_cascade_evolution.pdf}
  \caption{Perpendicular-cascade evolution for $B_y$ (top row) and $B_z$ (bottom row) at rollout steps 1, 5, 10, 25, 50. Walrus zero-shot (red, solid) departs from truth (black) progressively, with energy preferentially piling up at high $k_\perp$ by step 50. The high/low-$k$ ratio collapses from $\sim$150 (truth-like) at step~1 to $\sim$3 at step~50.}
  \label{fig:cascade-evolution}
\end{figure}
