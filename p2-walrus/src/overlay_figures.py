...
configs on the same shifted window (frames [3..52]):
    ...
    fig_per_traj.pdf           - per-trajectory VRMSE, all 10 trajectories
                                 across all 4 methods overlaid in a single
                                 panel with a method legend.
...
def fig_per_traj(root: Path):
    """Single overlaid panel: all 10 per-trajectory VRMSE traces for each
    of the four methods plotted in the same axes, colored by method."""
    ...
    for cfg in CONFIGS:
        ...
        for i in range(v.shape[0]):
            ax.plot(x, v[i], color=COLORS[cfg], ls=LINESTYLES[cfg],
                    lw=0.8, alpha=0.55)
        # Median across the 10 trajectories, drawn solid on top.
        ax.plot(x, np.median(v, 0), color=COLORS[cfg], ls=LINESTYLES[cfg],
                lw=2.0, alpha=0.95)
        legend_handles.append(
            Line2D([0], [0], color=COLORS[cfg], ls=LINESTYLES[cfg], lw=2.0,
                   label=LABELS[cfg])
        )
    ax.set_yscale("log")
    ax.set_xlabel("rollout step")
    ax.set_ylabel("VRMSE")
    ax.set_title("Per-trajectory rollout VRMSE: all 10 trajectories per method")
    ax.grid(alpha=0.3, which="both")
