...
configs on the same shifted window (frames [3..52]):
    ...
    fig_per_traj.pdf           - per-trajectory VRMSE (single overlaid panel)
...
def fig_per_traj(root: Path):
    """Single overlaid panel: per-trajectory VRMSE for all 4 configurations
    drawn on the same axes, with a legend distinguishing the configurations.

    Thin lines = individual trajectories (one per traj, colored by config);
    thick lines = config mean; legend keyed by config color.
    """
    ...
    for cfg in CONFIGS:
        ...
        # Per-trajectory thin traces
        for i in range(v.shape[0]):
            ax.plot(x, v[i], color=COLORS[cfg], lw=0.5, alpha=0.35,
                    ls=LINESTYLES[cfg])
        # Config-mean thick trace (used in legend)
        line, = ax.plot(x, v.mean(0), color=COLORS[cfg], lw=2.0,
                        ls=LINESTYLES[cfg], label=LABELS[cfg])
    ...
    ax.legend(handles=legend_handles, fontsize=8, loc="upper left",
              title="configuration (thick=mean, thin=per-trajectory)")
    fig.tight_layout()
    return fig
